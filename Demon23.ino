...
//    GAR's R clears?
//  X Bonus countdown not quick enough
//  X Spinner sounds
//    Swap default boot
//    Do lamp sweeps
//    Drops scores on reset
//    Magnet lamps
//  X Magnet switch
...
#define SOUND_EFFECT_DROP_TARGET_HIT          2
#define SOUND_EFFECT_DROP_TARGET_COMPLETE     3
#define SOUND_EFFECT_HEARTBEAT                4
#define SOUND_EFFECT_SAUCER_NO_VALUE          5
#define SOUND_EFFECT_SAUCER_COLLECT           6
#define SOUND_EFFECT_SPINNER_UNLIT            7
#define SOUND_EFFECT_SPINNER_LIT              8
#define SOUND_EFFECT_POP_BUMPER_UNLIT         9
#define SOUND_EFFECT_POP_BUMPER_LIT           10
#define SOUND_EFFECT_SLING_SHOT               11
#define SOUND_EFFECT_GAME_START               12
#define SOUND_EFFECT_10_PT                    13
#define SOUND_EFFECT_TOP_LANE_LIT             14
#define SOUND_EFFECT_TOP_LANE_UNLIT           15
#define SOUND_EFFECT_TOP_LANES_CLEARED        16
#define SOUND_EFFECT_BALL_OVER                19
#define SOUND_EFFECT_GAME_OVER                20
#define SOUND_EFFECT_SKILL_SHOT_MADE          21
#define SOUND_EFFECT_SKILL_SHOT_MISSED        22
#define SOUND_EFFECT_TILT_WARNING             28
#define SOUND_EFFECT_MATCH_SPIN               30
#define SOUND_EFFECT_STANDUP_LIT              31
#define SOUND_EFFECT_STANDUP_UNLIT            32
#define SOUND_EFFECT_STANDUPS_COMPLETED       33
#define SOUND_EFFECT_BEAT_GORGAR              34
...
byte SkillShotLane;
byte LaneStatus[4];       // A, B, C, D, E
byte StandupStatus[4];    // 1, 2, 3, 4
byte StandupSpecial[4];
byte StandupsCompleted[4];
byte SaucerStatus[4];
byte GorgarStatus[4];
byte SnakePitStatus[4];
byte GorStatus[4];
byte GarStatus[4];
...
unsigned long TopLaneAnimationStartTime;
unsigned long InlaneAnimationStartTime;
unsigned long StandupAnimationStartTime;
unsigned long SnakepitAnimationStartTime;
unsigned long SkillShotAnimationStartTime;
unsigned long TurnOffMagnetTime;
unsigned long TurnOffSnakePitLampTime;
unsigned long GORTargetsCompletedTime;
unsigned long GARTargetsCompletedTime;

DropTargetBank GORTargets(3, 1, DROP_TARGET_TYPE_WLLMS_1, 50);
DropTargetBank GARTargets(3, 1, DROP_TARGET_TYPE_WLLMS_1, 50);


#define LANE_A_BIT    0x01
#define LANE_B_BIT    0x02
#define LANE_C_BIT    0x04
#define LANE_ALL_TOP  0x07
#define LANE_D_BIT    0x08
#define LANE_E_BIT    0x10
#define LANE_BOTH_INLANES 0x18

#define GORGAR_STATUS_GOR_LIT           0x01
#define GORGAR_STATUS_GAR_LIT           0x02
#define GORGAR_STATUS_GORGAR_COMPLETE   0x03
#define GORGAR_STATUS_LEVEL_UP          0x04
#define GORGAR_STATUS_LEVEL_1           0x04
#define GORGAR_STATUS_LEVEL_2           0x08
#define GORGAR_STATUS_LEVEL_3           0x0C

#define SNAKEPIT_STATUS_5K                1
#define SNAKEPIT_STATUS_10K               2
#define SNAKEPIT_STATUS_20K               3
#define SNAKEPIT_STATUS_30K               4
#define SNAKEPIT_STATUS_50K               5
#define SNAKEPIT_STATUS_50K_FROM_NUMBERS  6
...
void setup() {
  ...
  initResult = RPU_InitializeMPU(   RPU_CMD_BOOT_ORIGINAL_IF_NOT_CREDIT_RESET | RPU_CMD_BOOT_ORIGINAL_IF_NOT_SWITCH_CLOSED |
  ...
  GORTargets.DefineSwitch(0, SW_GOR_G);
  GORTargets.DefineSwitch(1, SW_GOR_O);
  GORTargets.DefineSwitch(2, SW_GOR_R);
//  GORTargets.AddAllTargetsSwitch(SW_GOR_COMPLETE);
  GORTargets.DefineResetSolenoid(0, SOL_GOR_RESET);

  GARTargets.DefineSwitch(0, SW_GAR_G);
  GARTargets.DefineSwitch(1, SW_GAR_A);
  GARTargets.DefineSwitch(2, SW_GAR_R);
//  GARTargets.AddAllTargetsSwitch(SW_GAR_COMPLETE);
  GARTargets.DefineResetSolenoid(0, SOL_GAR_RESET);
...
void ShowLaneLamps() {
  ...
  if (GameMode==GAME_MODE_SKILL_SHOT) {
    RPU_SetLampState(LAMP_A, (SkillShotLane==0)||(LaneStatus[CurrentPlayer]&LANE_A_BIT), 0, (SkillShotLane==0)?600:0);
    RPU_SetLampState(LAMP_B, (SkillShotLane==1)||(LaneStatus[CurrentPlayer]&LANE_B_BIT), 0, (SkillShotLane==1)?600:0);
    RPU_SetLampState(LAMP_C, (SkillShotLane==2)||(LaneStatus[CurrentPlayer]&LANE_C_BIT), 0, (SkillShotLane==2)?600:0);
    RPU_SetLampState(LAMP_E, 0);
    RPU_SetLampState(LAMP_E, 0);
  } else if (GameMode==GAME_MODE_UNSTRUCTURED_PLAY) {
    if (SkillShotAnimationStartTime) {
      RPU_SetLampState(LAMP_A, (SkillShotLane==0), 0, 100);
      RPU_SetLampState(LAMP_B, (SkillShotLane==1), 0, 100);
      RPU_SetLampState(LAMP_C, (SkillShotLane==2), 0, 100);
      if (CurrentTime>(SkillShotAnimationStartTime+1000)) {
        SkillShotAnimationStartTime = 0;
      }
    } else if (TopLaneAnimationStartTime) {
      byte lanePhase = ((CurrentTime-TopLaneAnimationStartTime)/100)%3;
      RPU_SetLampState(LAMP_A, lanePhase==0);
      RPU_SetLampState(LAMP_B, lanePhase==1);
      RPU_SetLampState(LAMP_C, lanePhase==2);
      if (CurrentTime>(TopLaneAnimationStartTime+2500)) {
        TopLaneAnimationStartTime = 0;
      }
    } else {
      RPU_SetLampState(LAMP_A, LaneStatus[CurrentPlayer] & LANE_A_BIT);
      RPU_SetLampState(LAMP_B, LaneStatus[CurrentPlayer] & LANE_B_BIT);
      RPU_SetLampState(LAMP_C, LaneStatus[CurrentPlayer] & LANE_C_BIT);
    }

    if (InlaneAnimationStartTime) {
      byte lanePhase = ((CurrentTime-InlaneAnimationStartTime)/250)%2;
      RPU_SetLampState(LAMP_D, lanePhase==0);
      RPU_SetLampState(LAMP_E, lanePhase==1);
      if (CurrentTime>(InlaneAnimationStartTime+2500)) {
        InlaneAnimationStartTime = 0;
      }
    } else {
      RPU_SetLampState(LAMP_D, LaneStatus[CurrentPlayer] & LANE_D_BIT);
      RPU_SetLampState(LAMP_E, LaneStatus[CurrentPlayer] & LANE_E_BIT);
    }
  }
}
...
void ShowBonusLamps() {
  if (GameMode==GAME_MODE_SKILL_SHOT) {
    if ( (CurrentTime-LastHeartbeatTime)<550 ) {
      byte heartbeatPhase = (((CurrentTime-LastHeartbeatTime)/50)%11) + 1;
      for (byte count=1; count<=10; count++) {
        if (count==9) {
          RPU_SetLampState(LAMP_BONUS_9, heartbeatPhase==9 || heartbeatPhase==10);
        } else if (count==10) {
          RPU_SetLampState(LAMP_BONUS_10, heartbeatPhase==10 || heartbeatPhase==11);
        } else {
          RPU_SetLampState(LAMP_BONUS_1+(count-1), count==heartbeatPhase || count==(heartbeatPhase-1));
        }
      }
    } else {
      for (byte count=0; count<9; count++) RPU_SetLampState(LAMP_BONUS_1+count, 0);
      RPU_SetLampState(LAMP_BONUS_10, 0);
    }
  } else if (GameMode==GAME_MODE_UNSTRUCTURED_PLAY) {
    byte bonusToShow = Bonus[CurrentPlayer];
    if (bonusToShow>20) {
      RPU_SetLampState(LAMP_BONUS_20, 1);
      bonusToShow -= 20;
    } else {
      RPU_SetLampState(LAMP_BONUS_20, 0);
    }
    if (bonusToShow>10) {
      RPU_SetLampState(LAMP_BONUS_10, 1);
      bonusToShow -= 10;
    } else {
      RPU_SetLampState(LAMP_BONUS_10, 0);
    }
    ...
    for (byte count=1; count<10; count++) {
      RPU_SetLampState(LAMP_BONUS_1 + (count-1), count<=bonusToShow);
    }
...
void ShowSnakePitLamps() {
  if (SnakePitStatus[CurrentPlayer]==SNAKEPIT_STATUS_50K_FROM_NUMBERS) {
    byte lampPhase = (CurrentTime/200)%5;
    for (byte count=0; count<5; count++) {
      RPU_SetLampState(LAMP_MAGNET_5+count, lampPhase>=count);  
    }
  } else {
    for (byte count=0; count<5; count++) {
      RPU_SetLampState(LAMP_MAGNET_5+count, SnakePitStatus[CurrentPlayer]>count, 0, ((SnakePitStatus[CurrentPlayer]-1)==count)?150:0);
    }
  }
}
...
void ShowStandupLamps() {
  ...
  if (StandupAnimationStartTime) {
    byte lampPhase = (CurrentTime/100)%4;
    RPU_SetLampState(LAMP_1, lampPhase==0);
    RPU_SetLampState(LAMP_2, lampPhase==1);
    RPU_SetLampState(LAMP_3, lampPhase==2);
    RPU_SetLampState(LAMP_4, lampPhase==3);

    if (CurrentTime>(StandupAnimationStartTime+3000)) {
      StandupAnimationStartTime = 0;
    }
  } else { 
    byte lampPhase = (CurrentTime/100)%2;
    if (StandupStatus[CurrentPlayer]&0x10) RPU_SetLampState(LAMP_1, lampPhase);
    else RPU_SetLampState(LAMP_1, (StandupStatus[CurrentPlayer]&0x01));
    
    if (StandupStatus[CurrentPlayer]&0x20) RPU_SetLampState(LAMP_2, lampPhase==0);
    else RPU_SetLampState(LAMP_2, (StandupStatus[CurrentPlayer]&0x02));
    
    if (StandupStatus[CurrentPlayer]&0x40) RPU_SetLampState(LAMP_3, lampPhase);
    else RPU_SetLampState(LAMP_3, (StandupStatus[CurrentPlayer]&0x04));
    
    if (StandupStatus[CurrentPlayer]&0x80) RPU_SetLampState(LAMP_4, lampPhase==0);
    else RPU_SetLampState(LAMP_4, (StandupStatus[CurrentPlayer]&0x08));
  }
}
...
void ShowDropTargetLamps() {
  ...
  if (GameMode==GAME_MODE_SKILL_SHOT) {
    RPU_SetLampState(LAMP_GOR, 0);
    RPU_SetLampState(LAMP_GAR, 0);
  } else {
    RPU_SetLampState(LAMP_GOR, GorgarStatus[CurrentPlayer] & GORGAR_STATUS_GOR_LIT);
    RPU_SetLampState(LAMP_GAR, GorgarStatus[CurrentPlayer] & GORGAR_STATUS_GAR_LIT);
  }
...
void ShowSpinnerLamp() {
  if (GameMode==GAME_MODE_SKILL_SHOT) {
    ...
    RPU_SetLampState(LAMP_SPINNER, SpinnerLit);
  }
...
void ShowPopBumperLamps() {
  RPU_SetLampState(LAMP_TOP_POP_BUMPER, GarStatus[CurrentPlayer]>1);
  RPU_SetLampState(LAMP_LEFT_POP_BUMPER, GarStatus[CurrentPlayer]);
  RPU_SetLampState(LAMP_BOTTOM_POP_BUMPER, GarStatus[CurrentPlayer]>2);
}


void ShowSaucerLamps() {
  if (GameMode==GAME_MODE_SKILL_SHOT) {
    RPU_SetLampState(LAMP_SAUCER_10, 0);
    RPU_SetLampState(LAMP_SAUCER_15, 0);
    RPU_SetLampState(LAMP_SAUCER_EXTRA_BALL, 0);
  } else {
    RPU_SetLampState(LAMP_SAUCER_10, GorgarStatus[CurrentPlayer] & GORGAR_STATUS_LEVEL_1);
    RPU_SetLampState(LAMP_SAUCER_15, GorgarStatus[CurrentPlayer] & GORGAR_STATUS_LEVEL_2);
    RPU_SetLampState(LAMP_SAUCER_EXTRA_BALL, (GorgarStatus[CurrentPlayer] & GORGAR_STATUS_LEVEL_3)==GORGAR_STATUS_LEVEL_3);
  }
}
...
*/
      case SOUND_EFFECT_NONE:
        Audio.PlaySoundCardWhenPossible(19*256, CurrentTime, 0, 50, 100);
        break;

      case SOUND_EFFECT_BEAT_GORGAR:
        Audio.PlaySoundCardWhenPossible(25*256, CurrentTime, 0, 50, 100);
        break;

      case SOUND_EFFECT_SKILL_SHOT_MADE:
        Audio.PlaySoundCardWhenPossible(7*256, CurrentTime, 0, 50, 100);
        break;

      case SOUND_EFFECT_SKILL_SHOT_MISSED:
        Audio.PlaySoundCardWhenPossible(6*256, CurrentTime, 0, 50, 100);
        break;

      case SOUND_EFFECT_10_PT:
      case SOUND_EFFECT_SLING_SHOT:
        Audio.PlaySoundCardWhenPossible(5*256, CurrentTime, 0, 250, 5);
        break;

      case SOUND_EFFECT_DROP_TARGET_HIT:
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 0, 250, 10);
        break;

      case SOUND_EFFECT_DROP_TARGET_COMPLETE:
        Audio.PlaySoundCardWhenPossible(21*256, CurrentTime, 0, 70, 10);
        Audio.PlaySoundCardWhenPossible(21*256, CurrentTime, 75, 70, 10);
        Audio.PlaySoundCardWhenPossible(21*256, CurrentTime, 150, 70, 10);
        Audio.PlaySoundCardWhenPossible(21*256, CurrentTime, 225, 70, 10);
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 300, 70, 10);
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 375, 70, 10);
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 450, 70, 10);
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 525, 70, 10);
        break;

      case SOUND_EFFECT_GAME_START:
        Audio.PlaySoundCardWhenPossible(24*256, CurrentTime, 0, 2000, 100);
        break;

      case SOUND_EFFECT_BONUS_COUNT:
        Audio.PlaySoundCardWhenPossible(22*256, CurrentTime, 0, 250, 10);
        break;        

      case SOUND_EFFECT_SPINNER_LIT:
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 0, 250, 10);
        break;        

      case SOUND_EFFECT_SPINNER_UNLIT:
        Audio.PlaySoundCardWhenPossible(21*256, CurrentTime, 0, 250, 10);
        break;        

      case SOUND_EFFECT_TOP_LANE_LIT:
      case SOUND_EFFECT_TOP_LANE_UNLIT:
        Audio.PlaySoundCardWhenPossible(15*256, CurrentTime, 0, 250, 10);
        break;

      case SOUND_EFFECT_POP_BUMPER_UNLIT:
      case SOUND_EFFECT_POP_BUMPER_LIT:
        Audio.PlaySoundCardWhenPossible(21*256, CurrentTime, 0, 750, 10);
        break;
    
      case SOUND_EFFECT_HEARTBEAT:
        //Audio.PlaySoundCardWhenPossible(19*256, CurrentTime, 0, 19, 0);
        Audio.PlaySoundCardWhenPossible(18*256, CurrentTime, 0, 1000, 5);
        break;
      ...
      case SOUND_EFFECT_MATCH_SPIN:
        Audio.PlaySoundCardWhenPossible(3 * 256, CurrentTime, 0, 25, 5);
        break;
...
int RunAttractMode(int curState, boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    // playing sound
    PlaySoundEffect(SOUND_EFFECT_NONE);
    RPU_DisableSolenoidStack();
    RPU_SetContinuousSolenoid(false, SOL_MAGNET_RELAY);
    RPU_SetContinuousSolenoid(false, SOL_MAGNET_LAMPS);
...
int InitGamePlay(boolean curStateChanged) {
  ...
  if (CountBallsInTrough()==0 && WaitForBallToReachOuthole) {
  ...
  for (int count = 0; count < 4; count++) {
    ...
    SaucerStatus[count] = 0;
    GorgarStatus[count] = 0;
    SnakePitStatus[count] = 0;
    StandupSpecial[count] = 0;
    StandupsCompleted[count] = 0;
    GorStatus[count] = 0;
    GarStatus[count] = 0;
  }
...
int InitNewBall(bool curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    InlaneAnimationStartTime = 0;
    StandupAnimationStartTime = 0;
    SnakepitAnimationStartTime = 0;
    SkillShotAnimationStartTime = 0;
    TurnOffMagnetTime = 0;
    TurnOffSnakePitLampTime = 0;
    SpinnerLit = false;

    // Reset Drop Targets
    GORTargets.ResetDropTargets(CurrentTime + 100, true);
    GARTargets.ResetDropTargets(CurrentTime + 500, true);
...
void UpdateDropTargets() {
  GORTargets.Update(CurrentTime);
  GARTargets.Update(CurrentTime);
}
...
int ManageGameMode() {
  ...
  switch ( GameMode ) {
    case GAME_MODE_SKILL_SHOT:
      ...
      // Show heartbeat
      if (GameModeStage==1 && CurrentTime>(LastHeartbeatTime+HeartbeatDelay)) {
        LastHeartbeatTime = CurrentTime;
      }

      // Play heartbeat
      if (CurrentTime>(LastTimePromptPlayed+3000) && GameModeStage==0) {
        GameModeStage = 1;
        PlaySoundEffect(SOUND_EFFECT_HEARTBEAT);
      }
    ...
    case GAME_MODE_UNSTRUCTURED_PLAY:
      ...
      if ((Bonus[CurrentPlayer]%20)>=10) SpinnerLit = true;
      else SpinnerLit = false;
...
int CountDownDelayTimes[] = {75, 75, 75, 50, 50, 50, 50, 50};
...
boolean HandleDropTarget(byte switchHit) {
  ...
  if (DEBUG_MESSAGES) {
    char buf[128];
    sprintf(buf, "GOR switch hit = %d\n", switchHit);
    Serial.write(buf);
  }

  // Ignore drop switch pulses that are right after start of ball
  if (GameMode==GAME_MODE_SKILL_SHOT && CurrentTime<(GameModeStartTime+1000)) return false;

  if (switchHit>=SW_GOR_G && switchHit<=SW_GOR_COMPLETE) {
    result = GORTargets.HandleDropTargetHit(switchHit);
    numTargetsDown = (unsigned long)CountBits(result);

    if (numTargetsDown) {
      CurrentScores[CurrentPlayer] += PlayfieldMultiplier * numTargetsDown * 100;
    
      boolean cleared = GORTargets.CheckIfBankCleared();
      if (cleared) {
        if (DEBUG_MESSAGES) {
          char buf[128];
          sprintf(buf, "GOR Status = 0x%02X\n", GORTargets.GetStatus(false));
          Serial.write(buf);
        }
        GORTargets.ResetDropTargets(CurrentTime + 500, true);
        PlaySoundEffect(SOUND_EFFECT_DROP_TARGET_COMPLETE);
        GorgarStatus[CurrentPlayer] |= GORGAR_STATUS_GOR_LIT;
        if (GorStatus[CurrentPlayer]<250) GorStatus[CurrentPlayer] += 1;
        if (SnakePitStatus[CurrentPlayer]<SNAKEPIT_STATUS_50K) SnakePitStatus[CurrentPlayer] += 1;
      } else {
        PlaySoundEffect(SOUND_EFFECT_DROP_TARGET_HIT);    
      }
    } else {
      if (DEBUG_MESSAGES) Serial.write("GOR duplicate hit\n");
    }
  }

  if (switchHit>=SW_GAR_G && switchHit<=SW_GAR_COMPLETE) {
    result = GARTargets.HandleDropTargetHit(switchHit);
    numTargetsDown = (unsigned long)CountBits(result);

    if (numTargetsDown) {
      CurrentScores[CurrentPlayer] += PlayfieldMultiplier * numTargetsDown * 100;
    
      boolean cleared = GARTargets.CheckIfBankCleared();
      if (cleared) {
        GARTargets.ResetDropTargets(CurrentTime + 500, true);
        PlaySoundEffect(SOUND_EFFECT_DROP_TARGET_COMPLETE);
        GorgarStatus[CurrentPlayer] |= GORGAR_STATUS_GAR_LIT;
        if (GarStatus[CurrentPlayer]<250) GarStatus[CurrentPlayer] += 1;
      } else {
        PlaySoundEffect(SOUND_EFFECT_DROP_TARGET_HIT);    
      }
    }
  ...
  if ( (GorgarStatus[CurrentPlayer]&GORGAR_STATUS_GORGAR_COMPLETE)==GORGAR_STATUS_GORGAR_COMPLETE ) {
    GorgarStatus[CurrentPlayer] &= ~GORGAR_STATUS_GORGAR_COMPLETE;
    GorgarStatus[CurrentPlayer] += GORGAR_STATUS_LEVEL_UP;
    if ( GorgarStatus[CurrentPlayer]>GORGAR_STATUS_LEVEL_3 ) {
      GorgarStatus[CurrentPlayer] = GORGAR_STATUS_LEVEL_3;
    }
  }

  if (numTargetsDown) return true;
  return false;
}


void HandleTopLane(byte switchHit) {

  byte switchIndex = switchHit - SW_A_LANE;
  byte switchBit = LANE_A_BIT<<switchIndex;
  boolean skillShotAwarded = false;
  boolean laneLit = (switchBit & LaneStatus[CurrentPlayer]) ? true:false;

  AddToBonus(1);  

  if (GameMode==GAME_MODE_SKILL_SHOT) {
    if (switchIndex==SkillShotLane) {
      StartScoreAnimation(25000 * PlayfieldMultiplier);
      PlaySoundEffect(SOUND_EFFECT_SKILL_SHOT_MADE);
      skillShotAwarded = true;
      SkillShotAnimationStartTime = CurrentTime;
    } else {
      PlaySoundEffect(SOUND_EFFECT_SKILL_SHOT_MISSED);
    }
  } else {
    if (laneLit) PlaySoundEffect(SOUND_EFFECT_TOP_LANE_LIT);
    else PlaySoundEffect(SOUND_EFFECT_TOP_LANE_UNLIT);
  }

  LaneStatus[CurrentPlayer] |= switchBit;

  if ( (LaneStatus[CurrentPlayer] & LANE_ALL_TOP)==LANE_ALL_TOP ) {
    LaneStatus[CurrentPlayer] = 0;
    TopLaneAnimationStartTime = CurrentTime;
    PlaySoundEffect(SOUND_EFFECT_TOP_LANES_CLEARED);
    IncreaseBonusX();
  }
...
void HandleStandupSwitch(byte switchHit) {
  byte switchIndex = 0;
  if (switchHit>=SW_2_STANDUP) switchIndex = 1 + (switchHit-SW_2_STANDUP);
  byte switchBit = 0x01 << switchIndex;

  if (StandupStatus[CurrentPlayer] & switchBit) {
    // This is already lit, so check for double light
    PlaySoundEffect(SOUND_EFFECT_STANDUP_LIT);
    CurrentScores[CurrentPlayer] += 1000;
    AddToBonus(1);

    StandupStatus[CurrentPlayer] |= switchBit * 16;
  } else {
    StandupStatus[CurrentPlayer] |= switchBit;
    PlaySoundEffect(SOUND_EFFECT_STANDUP_UNLIT);
    CurrentScores[CurrentPlayer] += 1000;
    AddToBonus(1);
  }

  if ( (StandupStatus[CurrentPlayer]&0x0F) == 0x0F ) {
    StandupAnimationStartTime = CurrentTime;
    PlaySoundEffect(SOUND_EFFECT_STANDUPS_COMPLETED);
    StandupsCompleted[CurrentPlayer] += 1;
    StandupStatus[CurrentPlayer] = 0;
    SnakePitStatus[CurrentPlayer] = SNAKEPIT_STATUS_50K_FROM_NUMBERS;
  }
...
void HandleGamePlaySwitches(byte switchHit) {
  ...
  switch (switchHit) {
    ...
    case SW_MAGNET_SWITCH:
      TurnOffMagnetTime = CurrentTime + 1500;
      RPU_SetContinuousSolenoid(true, SOL_MAGNET_RELAY);
      if (SnakePitStatus[CurrentPlayer]) {

        switch (SnakePitStatus[CurrentPlayer]) {
          case SNAKEPIT_STATUS_5K:
            StartScoreAnimation(5000*PlayfieldMultiplier);
            break;
          case SNAKEPIT_STATUS_10K:
            StartScoreAnimation(10000*PlayfieldMultiplier);
            break;
          case SNAKEPIT_STATUS_20K:
            StartScoreAnimation(20000*PlayfieldMultiplier);
            break;
          case SNAKEPIT_STATUS_30K:
            StartScoreAnimation(30000*PlayfieldMultiplier);
            break;
          case SNAKEPIT_STATUS_50K:
            StartScoreAnimation(50000*PlayfieldMultiplier);
            break;
          case SNAKEPIT_STATUS_50K_FROM_NUMBERS:
            StartScoreAnimation(50000*PlayfieldMultiplier);
            break;
        }

        PlaySoundEffect(SOUND_EFFECT_BEAT_GORGAR);
        if (SnakePitStatus[CurrentPlayer]>SNAKEPIT_STATUS_10K) {
          SnakePitStatus[CurrentPlayer] = 0;
        }
      }
    ...
    case SW_3_STANDUP:
    case SW_4_STANDUP:
      HandleStandupSwitch(switchHit);
      LastSwitchHitTime = CurrentTime;
      if (BallFirstSwitchHitTime == 0) BallFirstSwitchHitTime = CurrentTime;
      break;

    case SW_TOP_POP_BUMPER:
      if (GarStatus[CurrentPlayer]>1) {
        CurrentScores[CurrentPlayer] += 1000;
        PlaySoundEffect(SOUND_EFFECT_POP_BUMPER_LIT);
      } else {
        CurrentScores[CurrentPlayer] += 100;
        PlaySoundEffect(SOUND_EFFECT_POP_BUMPER_UNLIT);
      }
      LastSwitchHitTime = CurrentTime;
      if (BallFirstSwitchHitTime == 0) BallFirstSwitchHitTime = CurrentTime;
      break;

    case SW_LEFT_POP_BUMPER:
      if (GarStatus[CurrentPlayer]) {
        CurrentScores[CurrentPlayer] += 1000;
        PlaySoundEffect(SOUND_EFFECT_POP_BUMPER_LIT);
      } else {
        CurrentScores[CurrentPlayer] += 100;
    ...
    case SW_BOTTOM_POP_BUMPER:
      if (GarStatus[CurrentPlayer]>2) {
        CurrentScores[CurrentPlayer] += 1000;
        PlaySoundEffect(SOUND_EFFECT_POP_BUMPER_LIT);
      } else {
        CurrentScores[CurrentPlayer] += 100;
    ...
    case SW_SAUCER:
      ...
      PlaySoundEffect(SOUND_EFFECT_SAUCER_NO_VALUE);
...
int RunGamePlayMode(int curState, boolean curStateChanged) {
  ...
  if (TurnOffMagnetTime && CurrentTime>TurnOffMagnetTime) {
    RPU_SetContinuousSolenoid(false, SOL_MAGNET_RELAY);
    TurnOffMagnetTime = 0;
  }
  if (TurnOffSnakePitLampTime && CurrentTime>TurnOffSnakePitLampTime) {
    RPU_SetContinuousSolenoid(false, SOL_MAGNET_LAMPS);
    TurnOffSnakePitLampTime = 0;
  }
